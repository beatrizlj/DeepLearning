...
def main():
    ...
    # Dicionário para guardar estatísticas finais
    final_results = {
        'network': NETWORK,
        'loss_function': LOSS,
        'epochs': EPOCHS,
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(images, labels), 1):
        ...
        model = get_model(NETWORK, num_classes=len(class_names)).to(device)
        ...
        for epoch in range(1, EPOCHS + 1):
            print(f"Época {epoch}/{EPOCHS} [Fold {fold}]")
    ...
    # Fim da Validação Cruzada: Consolidar resultados
    ...
    save_json_info(final_results, os.path.join(MODELS_DIR, 'final_results.json'))
    print(f"Resultados, gráficos e pesos foram salvos em: {MODELS_DIR}")
